...
local function match_loop(context, dispatcher, tick, state, messages)
    ...
    nk.logger_info("📥 Количество юзерсов: " .. online_count)
    ...
    dispatcher.broadcast_message(
        EOpCode.OnlineCount,
        payload,
        nil,        -- sender (можно nil, если не важно)
        nil,        -- reliable (nil = по умолчанию true)
        { sender }  -- recipients (таблица из одного отправителя)
    )
end
    end
